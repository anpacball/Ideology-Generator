function calc() {
  ...
  if ((gardet / omlan) >= finetune) {
    output += "[!] Du kommer inte hinna, du har " + gardet + " omlån{DOT}";
    det_finns_tid = false;
  }
  else {
    output += "Detta betyder att du kommer hinna med " + gardet + " omlån{DOT}";
    det_finns_tid = true;
  }
  output += "<br>";
  ...
  table += '<th scope="col">' + '"Fine Tune"' + '</th></tr></thead><tbody>';
  ...
  while (x < 5) {
    ...
    table += (bokens_langd / sidor_pd) + '</td><td>';
    table += (bokens_langd / sidor_pd / lantid) + '</td>';
    if (finer >= finetune) {
      table += '<td>' + finer + ' >= ' + finetune + '</td></tr>';
    }
    else {
      table += '<td>' + finer + ' < ' + finetune + '</td></tr>';
    }
